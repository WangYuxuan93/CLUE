...
from metrics.sdp_compute_metrics import compute_metrics
#from processors.srl_loader import collate_fn, processors, load_and_cache_examples
from processors.sdp_processor import SdpProcessor, load_and_cache_examples, collate_fn
...
MODEL_CLASSES = {
    'sdp': (RobertaConfig, RobertaForSDP, AutoTokenizer),
}
...
def _prepare_inputs(inputs, device, use_dist=False, debug=False):
    ...
    if "src_heads" in inputs and inputs["src_heads"] is not None:
        inputs["src_heads"] = inputs["src_heads"].float()
    else:
        del inputs["src_heads"]
        del inputs["src_rels"]
...
def train(args, train_dataset, model, tokenizer):
    ...
    task_specific = ['input_encoder', 'arc', 'rel']
    no_decay = ['bias', 'LayerNorm.weight']
    task_specific_params = {'params': [p for n, p in model.named_parameters() if any(nd in n for nd in task_specific)],
         'weight_decay': args.weight_decay, 'lr': args.task_specific_lr}
    task_specific_param_names = [n for n, p in model.named_parameters() if any(nd in n for nd in task_specific)]
    print ("Task specific lr = {}, params:\n {}".format(args.task_specific_lr, task_specific_param_names))
    optimizer_grouped_parameters = [
        task_specific_params,
        {'params': [p for n, p in model.named_parameters() if not any(nd in n for nd in task_specific) and not any(nd in n for nd in no_decay)],
         'weight_decay': args.weight_decay},
        {'params': [p for n, p in model.named_parameters() if not any(nd in n for nd in task_specific) and any(nd in n for nd in no_decay)], 
         'weight_decay': 0.0}
    ]
...
def decode_sdp(arc_logits, rel_logits, must_has_head=False):
    arc_preds = arc_logits.ge(0.5).long()
    # (batch_size, seq_len, seq_len)
    rel_preds = rel_logits.argmax(-1)  # 在只有一个label的情况下找到最大的索引

    if must_has_head:
        # (batch_size, seq_len, seq_len)
        # use heads with max prob for those all heads' prob are lower than 0.5
        backoff_heads = _argmax(arc_logits)
        # (batch_size, seq_len, seq_len)
    ...
    rel_preds = arc_preds * rel_preds

    return arc_preds.detach().cpu().numpy(), rel_preds.detach().cpu().numpy()
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            with torch.no_grad():
                inputs = _prepare_inputs(batch, args.device)
                outputs = model(**inputs)
                tmp_eval_loss, arc_logits, rel_logits = outputs[:3]
                eval_loss += tmp_eval_loss.mean().item()
            nb_eval_steps += 1
            #print ("logits: ", logits.size())
            #print ("labels: ", inputs['labels'].size())
            _arc_preds, _rel_preds = decode_sdp(arc_logits, rel_logits)
            pred_heads.extend([l for l in _arc_preds])
            pred_rels.extend([l for l in _rel_preds])
            gold_heads.extend([l for l in inputs['heads'].detach().cpu().numpy()])
            gold_rels.extend([l for l in inputs['rels'].detach().cpu().numpy()])
            pbar(step)
        print(' ')
        if 'cuda' in str(args.device):
            torch.cuda.empty_cache()
        ...
        result = compute_metrics(pred_heads, pred_rels, gold_heads, gold_rels)
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        ...
        for step, batch in enumerate(pred_dataloader):
            ...
            with torch.no_grad():
                inputs = _prepare_inputs(batch, args.device)
                outputs = model(**inputs)
                _, arc_logits, rel_logits = outputs[:3]
            nb_pred_steps += 1
            _arc_preds, _rel_preds = decode_sdp(arc_logits, rel_logits)
            pred_heads.extend([l for l in _arc_preds])
            pred_rels.extend([l for l in _rel_preds])
        ...
        # 保存标签结果
        write_sdp_conllu(pred_heads, pred_rels, pred_examples, output_submit_file, label_map)
...
def main(): 
    ...
    parser.add_argument("--task_name", default='sdp', type=str, required=True,
    ...
    parser.add_argument("--task_specific_lr", default=1e-2, type=float,
                        help="The initial learning rate for task specific params.")
    ...
    processor = SdpProcessor(args.task_name)
    ...
    if args.do_train:
        ...
        if args.official_syntax_type:
            config.graph["num_rel_labels"] = len(processor.get_source_label_map())
