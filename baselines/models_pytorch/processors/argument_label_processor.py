...
from processors.srl_processor import flatten_heads_to_matrix, prepare_word_level_input
...
class ArgumentLabelProcessor(SrlProcessor):
    ...
    def get_labels(self):
        ...
        if self.task.startswith('conll09-zh'):
            ...
            self.label_map = upb_chinese_label_mapping
...
class ArgumentLabelProcessorSDP(ArgumentLabelProcessor):
    def __init__(self, task):
        super().__init__(task)
...
def convert_parsed_examples_to_features_sdp(
    ...
    """
    processor = ArgumentLabelProcessorSDP(task)
    if label_list is None:
        label_list = processor.get_labels()
        logger.info("Using label list %s for task %s" % (label_list, task))
