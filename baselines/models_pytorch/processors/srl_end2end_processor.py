...
def load_and_cache_examples(args, task, tokenizer, data_type='train', return_examples=False):
    ...
    if args.official_syntax_type == "sdp":
        processor = SrlEnd2EndProcessorSDP(task)
    else:
        processor = SrlEnd2EndProcessor(task)
...
class SrlEnd2EndProcessorSDP(SrlEnd2EndProcessor):
    def __init__(self, task):
        super().__init__(task)
        self.root_token = "<ROOT>"
    ...
    def _create_examples(self, sents, set_type, use_pos=False, is_test=False):
        ...
        for (i, sent) in enumerate(sents):
            ...
            heads = [0] + [x+1 for x in heads]
            rels = ['<PAD>'] + rels
            sdp_heads = [[0]] + [[x+1 for x in sdp_heads_] for sdp_heads_ in sdp_heads]
            sdp_rels = [['_<PAD>']] + sdp_rels
