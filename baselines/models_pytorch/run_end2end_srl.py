...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        result = compute_end2end_metrics(eval_task, preds, golds)
...
def predict(args, model, tokenizer, label_list, prefix="", is_ood=False):
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        ...
        result = compute_end2end_metrics(pred_task, preds, golds)
        logger.info("  Num examples = %d", len(pred_dataset))
        logger.info("  Batch size = %d", args.pred_batch_size)
        ...
        # 保存标签结果
        write_conll09_end2end(preds, golds, label_map, pred_examples, output_submit_file)
...
def get_arg_prd_mask(task):
    ...
    prd_mask = 1 - arg_mask
    # masked are -1e8, while valid are 0
    arg_mask = (1 - arg_mask) * -1e8
    prd_mask = (1 - prd_mask) * -1e8
    #print ("arg_mask:", arg_mask)
    #print ("prd_mask:", prd_mask)
    return torch.from_numpy(prd_mask), torch.from_numpy(arg_mask)

def get_predictions(logits, prd_mask, arg_mask, debug=False):
    logits = logits.detach().cpu()
    batch, seq_len, _, num_label = logits.size()

    prd_mask = prd_mask.unsqueeze(0).unsqueeze(1).expand(seq_len, 1, num_label)
    arg_mask = arg_mask.unsqueeze(0).unsqueeze(1).expand(seq_len, seq_len-1, num_label)

    #print ("arg_mask:", arg_mask)
    #print ("prd_mask:", prd_mask)
    mask = torch.cat([prd_mask, arg_mask], dim=1)
    torch.set_printoptions(profile="full")
    logits += mask
    preds = [l for l in logits.numpy().argmax(-1)]
...
def main(): 
    ...
    processor = SrlEnd2EndProcessor(args.task_name)
    ...
    if args.do_train:
        ...
        config.use_bilstm = args.use_bilstm
