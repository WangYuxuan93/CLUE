...
from metrics.re_compute_metrics import compute_metrics
from processors.re_processor import collate_fn, ReProcessor, load_and_cache_examples
...
from io_utils.re_writer import write_re
import shutil
import re
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            if preds is None:
                ...
                golds = np.append(golds, inputs['labels'].detach().cpu().numpy(), axis=0)
            pbar(step)
        print(' ')
        if 'cuda' in str(args.device):
            torch.cuda.empty_cache()
        ...
        result = compute_metrics(eval_task, preds, golds)
...
def predict(args, model, tokenizer, label_list, prefix="", is_ood=False):
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        ...
        preds = np.argmax(preds, axis=1)
        result = compute_metrics(pred_task, preds, golds)
        logger.info("  Num examples = %d", len(pred_dataset))
        logger.info("  Batch size = %d", args.pred_batch_size)
        ...
        # 保存标签结果
        write_re([label_map[p] for p in preds], pred_examples, output_submit_file)
...
def main(): 
    ...
    parser.add_argument("--task_name", default='finre', type=str, required=True, choices=['finre', 'sanwen'],
    ...
    args.data_dir = os.path.join(args.data_dir, args.task_name.lower())
    ...
    processor = ReProcessor(args.task_name)
