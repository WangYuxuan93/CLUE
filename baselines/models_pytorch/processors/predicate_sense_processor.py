...
from processors.srl_processor import align_flatten_heads_sdp, flatten_heads_to_matrix_sdp, InputConll09Example
...
class InputSemSynPredicateSenseFeatures(object):
    """A single set of features of data."""
...
# for sdp input heads/rels
class PredicateSenseProcessorSDP(PredicateSenseProcessor):
    def __init__(self, task):
        super().__init__(task)

    def get_syntax_label_map(self):
        self.syntax_label_map = chinese_sdp_lv3_label_mapping
        ...
        for (i, sent) in enumerate(sents):
            ...
            examples.append(
                InputConll09Example(guid=guid, sid=sid, words=words, pred_ids=pred_ids, 
                                ...
                                pred_heads=sdp_heads, pred_rels=sdp_rels))
        return examples


def convert_parsed_examples_to_features_sdp(
        examples, 
        tokenizer,
        parser,
        max_length=512,
    ...
    """
    processor = PredicateSenseProcessorSDP(task)
    if label_list is None:
        label_list = processor.get_labels()
        logger.info("Using label list %s for task %s" % (label_list, task))
    ...
    if is_word_level:
        ...
        heads, rels = align_flatten_heads_sdp(
                        attention_mask=tokenized_inputs['attention_mask'],
                        word_ids=[tokenized_inputs.word_ids(i) for i in range(len(examples))],
                        flatten_heads=[example.pred_heads for example in examples],
                        flatten_rels=[example.pred_rels for example in examples],
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if official_syntax_type == "sdp":
            ...
            features.append(
                InputSemSynPredicateSenseFeatures(input_ids=tokenized_inputs['input_ids'][ex_index],
                              attention_mask=tokenized_inputs['attention_mask'][ex_index],
                              token_type_ids=tokenized_inputs['token_type_ids'][ex_index],
                              predicate_mask=predicate_mask,
                              label_ids=label_ids,
