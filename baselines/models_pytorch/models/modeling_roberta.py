...
class RobertaForWordLevelArgumentLabel(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if word_mask is not None:
                active_loss = word_mask.contiguous().view(-1) == 1
                active_logits = logits.view(-1, self.num_labels)
                active_labels = torch.where(
                    active_loss, labels.contiguous().view(-1), torch.tensor(loss_fct.ignore_index).type_as(labels)
                )
...
class RobertaForWordLevelPredicateSense(RobertaPreTrainedModel):
...
class RobertaForEnd2EndSrl(RobertaPreTrainedModel):
